process PREPARE_GENE_SPANS {
    ...
    with opener(in_path) as f:
        for line in f:
            ...
            # key on (gene_id, chrom): the same gene symbol can legitimately
            # appear on multiple chromosomes (paralogs/pseudogenes, e.g.
            # span, not a silent drop of every copy after the first
            key = (gene_id, chrom)
            if key not in genes:
                genes[key] = {
                    'chrom': chrom,
